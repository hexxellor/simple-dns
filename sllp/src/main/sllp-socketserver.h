...
#define SERVER_PRIVATE_METHOD						\
    ...
    int32_t (*__verify_request)	(struct sllp_server*, struct sllp_socket* request); \
    int32_t (*__server_close)	(struct sllp_server*);			\
    int32_t (*__process_request)(struct sllp_server*, struct sllp_socket* request); \
    int32_t (*__close_request)	(struct sllp_server*, struct sllp_socket* request); \
    int32_t (*__handle_error)	(struct sllp_server*, struct sllp_socket* request); \
